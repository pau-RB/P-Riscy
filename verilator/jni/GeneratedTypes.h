...
typedef enum IfcNames { IfcNamesNone=0, PlatformIfcNames_MemServerRequestS2H=1, PlatformIfcNames_MMURequestS2H=2, PlatformIfcNames_MemServerIndicationH2S=3, PlatformIfcNames_MMUIndicationH2S=4, IfcNames_ToHostH2S=5, IfcNames_FromHostS2H=6,  } IfcNames;
...
enum { CHAN_NUM_FromHost_startPC};
extern const uint32_t FromHost_reqinfo;

typedef struct {
    uint32_t pc;
} FromHost_startPCData;
typedef union {
    FromHost_startPCData startPC;
} FromHostData;
int FromHost_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd);
typedef struct {
    PORTAL_DISCONNECT disconnect;
    int (*startPC) (  struct PortalInternal *p, const uint32_t pc );
} FromHostCb;
extern FromHostCb FromHostProxyReq;
...
extern FromHostCb FromHostJsonProxyReq;

int ToHost_print ( struct PortalInternal *p, const uint32_t word );
enum { CHAN_NUM_ToHost_print};
extern const uint32_t ToHost_reqinfo;

typedef struct {
    uint32_t word;
} ToHost_printData;
typedef union {
    ToHost_printData print;
} ToHostData;
int ToHost_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd);
typedef struct {
    PORTAL_DISCONNECT disconnect;
    int (*print) (  struct PortalInternal *p, const uint32_t word );
} ToHostCb;
extern ToHostCb ToHostProxyReq;
...
extern ToHostCb ToHostJsonProxyReq;
#ifdef __cplusplus
}
#endif
