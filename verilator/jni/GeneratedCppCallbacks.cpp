...
extern const uint32_t ifcNames_ToHostH2S = IfcNames_ToHostH2S;
extern const uint32_t ifcNames_FromHostS2H = IfcNames_FromHostS2H;
...
/************** Start of FromHostWrapper CPP ***********/
#include "FromHost.h"
int FromHostdisconnect_cb (struct PortalInternal *p) {
    (static_cast<FromHostWrapper *>(p->parent))->disconnect();
    return 0;
};
int FromHoststartPC_cb (  struct PortalInternal *p, const uint32_t pc ) {
    (static_cast<FromHostWrapper *>(p->parent))->startPC ( pc);
    return 0;
};
FromHostCb FromHost_cbTable = {
    FromHostdisconnect_cb,
    FromHoststartPC_cb,
};

/************** Start of ToHostWrapper CPP ***********/
#include "ToHost.h"
int ToHostdisconnect_cb (struct PortalInternal *p) {
    (static_cast<ToHostWrapper *>(p->parent))->disconnect();
    return 0;
};
int ToHostprint_cb (  struct PortalInternal *p, const uint32_t word ) {
    (static_cast<ToHostWrapper *>(p->parent))->print ( word);
    return 0;
};
ToHostCb ToHost_cbTable = {
    ToHostdisconnect_cb,
    ToHostprint_cb,
};
